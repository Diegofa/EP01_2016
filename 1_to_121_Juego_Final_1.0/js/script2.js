//Variables usadas por la funcion cronometro
...
//La funcion del cronometro fue tomada de internet y efunciona con Jquery
...
var n_aleatorios = (function n_aleatorios(maximo_aleatorios)
{
  	...
  	for (var i = 0; i < dimension; i++)
  	{
    	...
    	for (var j = 0; j < dimension; j++)
    	{
          ...
          //celda.setAttribute('align', 'center');
          	...
          	//A  la celda se le asigna un id con el mismo numero que se genera dentro de ella para ello el setAtribute ("id")
  ...
  //Funcion que valida que el numero presionado siga el orden de 1 a 121 
	$(".cuadrado").on('click', function () {
		validate_number(this);
	})

	//Funcion que permite validar que solo se tengan 3 ayudas y ademas que siga la secuencia dle juego, dependiendo del numero que ya se haya presionado 
	$("#ayudas").on('click', function () {
		...
		hints++;
			...
			swal({  
			title: "No quedan ayudas!", 
		  	text: "Te has quedado sin ayudas",
		  	imageUrl: "./img/no.jpg" });;
		}
	})

// Muestra las instrucciones al presionar el boton ayudas 
	$("#help").on('click', function () 
	{
		swal({  
			title: "A jugar!", 
		  	text: "Debes encontrar los numeros en orden desde el 1 hasta el 121, tienes 3 ayudas; recuerda presionar el boton inicio para cronometrar el tiempo",
		  	imageUrl: "./img/mano.jpg" });					
	})

	$("#fin").on('click', function () 
	{
		swal({  
			title: "Nos veremos pronto!", 
		  	text: "Se recargará la pagina en 3sg",
		  	imageUrl: "./img/bye.gif" });
...
function startTimer() {
	if (timerCount == 0) {
		...
		jQuery('#btn').addClass("rgba(0, 0, 0, 0.5);");
		var time = getTime();
		timerAction(0);
		timerCount = 1;
	}
		...
		jQuery('#btn').css("height", "25px").css("vertical-align", "top");
		jQuery('#timerTxt').html('Descongelar').css("font-size", "14px");
		timerAction(1);
		timerCount = 0;
	}
}
